...
export default function ProfRegister() {
  ...
  const validate = () => {
    let tempErrors = {};
    tempErrors.username = formData.username ? "" : "El nombre de usuario es requerido.";
    ...
    tempErrors.birthDate = formData.birthDate ? "" : "La fecha de nacimiento es requerida.";
    tempErrors.password = formData.password.length >= 6 ? "" : "La contraseña debe tener al menos 6 caracteres.";
    tempErrors.repeatPassword = formData.password === formData.repeatPassword ? "" : "Las contraseñas no coinciden.";
    tempErrors.profesion = formData.profesion ? "" : "La profesión es requerida.";
    tempErrors.estado = formData.estado ? "" : "El estado es requerido.";
    tempErrors.profilePicture = profilePicture ? "" : "La foto de perfil es requerida.";
    tempErrors.officePhotos = officePhotos.length > 0 ? "" : "Al menos una foto de la oficina es requerida.";
  ...
  const handleRemovePhoto = (id) => {
    setOfficePhotos((prevPhotos) => prevPhotos.filter((photo) => photo.id !== id));
  };
  ...
  return (
      <div className="min-h-screen bg-gray-100">
        ...
        <div className="flex flex-col items-center justify-center pt-5">
          <form className="w-full lg:max-w-2xl max-w-md p-8 m-4 space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="lg:grid lg:grid-cols-2 lg:gap-6 lg:mb-6">
              <div className="mb-6 lg:mb-0">
                <div className="relative w-48 h-48 mx-auto overflow-hidden bg-gray-100 rounded-lg flex items-center justify-center border-2 border-gray-300 border-dashed">
                  <label htmlFor="profile-picture" className="flex flex-col items-center justify-center w-full h-full cursor-pointer bg-gray-50 hover:bg-gray-100">
                    <input id="profile-picture" type="file" className="hidden" onChange={handleProfilePictureChange} />
                    {errors.profilePicture && <p className="text-red-500 text-xs italic">{errors.profilePicture}</p>}
                    {profilePicture ? (
                      <img src={URL.createObjectURL(profilePicture)} alt="Profile Picture" className="w-full h-full object-cover rounded-lg" />
                    ) : (
              ...
              <div className="flex flex-col gap-3">
                <div>
                  ...
                  <Input icon={"FaUserAlt"} id="username" type="text" placeholder="tunombredeusuario" value={formData.username} onChange={handleInputChange} />
                  {errors.username && <p className="text-red-500 text-sm mt-1">{errors.username}</p>}
                </div>
                {/* Campo para el correo electrónico */}
                <div>
                  <Label htmlFor="email">Email</Label>
                  <Input icon={"MdEmail"} id="email" type="text" placeholder="email@example.com" value={formData.email} onChange={handleInputChange} />
                  {errors.email && <p className="text-red-500 text-sm mt-1">{errors.email}</p>}
                </div>
                <div>
                  {/* Campo para el nombre */}
                  <Label htmlFor="nombre">Nombre</Label>
                  <Input icon={"FaAddressCard"} id="nombre" type="text" value={formData.nombre} onChange={handleInputChange} placeholder="Tu primer nombre" />
                  {errors.nombre && <p className="text-red-500 text-sm mt-1">{errors.nombre}</p>}
                </div>
                <div>
                  {/* Campo para el apellido */}
                  <Label htmlFor="apellido">Apellido</Label>
                  <Input icon={"FaAddressCard"} id="apellido" type="text" value={formData.apellido} onChange={handleInputChange} placeholder="tu apellido" />
            ...
            {/* Campo para la fecha de nacimiento */}
                <div>
                  ...
                  <Input icon={"MdDateRange"} id="birthDate" type="date" placeholder="1991-01-01" value={formData.birthDate} onChange={handleInputChange} />
            ...
            <div className="lg:grid lg:grid-cols-2 lg:gap-6 lg:mb-6">
              <div className="flex flex-col gap-3">
                <div>
                  ...
                  <Input icon={"FaLock"} id="password" type="password" value={formData.password} onChange={handleInputChange} placeholder="••••••••" />
                  {errors.password && <p className="text-red-500 text-sm mt-1">{errors.password}</p>}
                </div>
                <div>
                  {/* Campo para repetir la contraseña */}
                  <Label htmlFor="repeatPassword">Repetir contraseña</Label>
                  <Input icon={"FaLock"} id="repeatPassword" type="password" value={formData.repeatPassword} onChange={handleInputChange} placeholder="••••••••" />
              ...
              <div className="mb-6 lg:mb-0">
                ...
                {errors.profesion && <p className="text-red-500 text-sm mt-1">{errors.profesion}</p>}
              </div>
            </div>
            ...
            <div className="lg:grid lg:grid-cols-2 lg:gap-6 lg:mb-6">
              <div className="gap-3">
                ...
                <div className="mt-4">
                  ...
                  <Checkbox label="Dar ubicación exacta" id="ubicacionExacta" checked={formData.ubicacionExacta} onChange={handleInputChange} />
                </div>
              </div>
            </div>
            {/* Sección del mapa */}
            <div>
              ...
              <div className="bg-white p-4 rounded-lg shadow-md">
                <MapComponent
                  ...
                  interactive={true}
                />
                {errors.officeAddresses && <p className="text-red-500 text-xs italic">{errors.officeAddresses}</p>}
              </div>
              <div className="mt-2 bg-white p-4 rounded-lg shadow-md">
                {/* Muestra las oficinas guardadas */}
                <h2 className="text-lg font-semibold mb-2">Oficinas Guardadas:</h2>
            ...
            <div>
              ...
              <Button className="relative flex items-center justify-center cursor-pointer py-1 px-3 text-sm" onClick={() => document.getElementById("office-photos").click()}>
                <FaPlusCircle className="h-4 w-4 mr-2" />
                Subir Fotos
              </Button>
              <input id="office-photos" type="file" className="hidden" onChange={handleFileChange} multiple accept="image/*" />
              {errors.officePhotos && <p className="text-red-500 text-xs italic">{errors.officePhotos}</p>}
              {/* Muestra las fotos de la oficina subidas */}
              <div className="flex space-x-2 my-4">
                {officePhotos.map((photo) => (
                  <div key={photo.id} className="relative w-20 h-20">
                    <img src={URL.createObjectURL(photo.file)} alt="Uploaded" className="w-full h-full object-cover rounded-lg" />
                    <button onClick={() => handleRemovePhoto(photo.id)} className="absolute inset-0 flex items-center justify-center bg-red-600 rounded-lg cursor-pointer bg-opacity-50 opacity-0 hover:opacity-90 transition-opacity">
